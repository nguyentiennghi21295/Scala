...
object HandlingFailure extends App{
  ...
  class Connection {
    ...
    def getSafe(url: String): Try[String] =
    {
      Try(get(url)) match {
        case s @Success(_) =>
          // Handle successful case, if needed
          s
        case f @ Failure(e) =>
          // Handle exception case, if needed
          // For example, you could log the error here
  ...
  object HttpService  {
    ...
    def getSafeConnection(host:String, port: String): Try[Connection] = Try(getConnection(host, port))
      .orElse(Failure(new RuntimeException("Failed to establish a safe connection!")))
  }
